...
public class ReportBuilder {
	...
	private TreeMap<Integer, String> reportedTestCases;
	...
	/** The SeleneseTestNgHelperVir */
	private SeleneseTestNgHelperVir selenseTestHelperVir;
	...
	public ReportBuilder(final String reportFolderLoc) {
		...
		rid = 0;
		uniqueTestCaseId = 0;
		reportedTestCases = new TreeMap<Integer, String>();
	}
	...
	public final void addNewTestCase(final String modulename, final String execDuration) {
		...
		String rId = getRid();
		testCase = new TestCase(modulename, execDuration, rId, getUniqueTestCaseId());

		// This is set Test Case to thread local object
		VtafThreadLocalClass.vtafThreadLocal.get().setCurrentTestCase(testCase);

		// This is to add Test Case to Test Suits.
		VtafThreadLocalClass.vtafThreadLocal.get().getCurrentTestSuit().getTestCases().add(testCase);

		/*
		 * for(TestSuite ts : testExecution.getTestSuites()){
		 * if(ts.getTestsuitename() != null &&
		 * ts.getTestsuitename().equals(testSuitName)){
		 * ts.getTestCases().add(testCase); break; } }
		 */
	 ...
	 *            the duration
	 */
	public final void addNewTestSuite(final String testSuiteName, final String duration) {
		...
		testSuite = new TestSuite(testSuiteName, iterationcount, maxchildren, duration, type, rId);
	...
	public final void addNewTestExecution() {
		...
		try {
			...
			String browser = selenseTestHelperVir.getBrowserString();
			user = System.getProperty("user.name");
			String machineName = InetAddress.getLocalHost().getHostName(); 
			language = (browser.toUpperCase(Locale.ENGLISH)).replace("*", "");
			host = machineName; // InetAddress.getLoopbackAddress().getHostName();
		...
		testExecution = new TestExecution(user, host, osversion, language, screenresolution, timestamp, duration);
	 ...
	 *            the loglvl
	 */
	public final void addNewTestStep(final boolean isPassed, final String category, final String message,
			final String loglvl) {
		...
		TestStep testStep = new TestStep(isPassed, time, category, message, loglvl);

		VtafThreadLocalClass.vtafThreadLocal.get().getCurrentTestCase().getTestSteps().add(testStep);
	 ...
	 *            the loglvl
	 */
	public final void addNewTestStep(final boolean isPassed, final String category, final String errimg,
			final String errthumb, final String message, final String stacktrace, final String loglvl) {

		TestStep testStep = new TestStep(isPassed, category, errimg, errthumb, message, stacktrace, loglvl);

		VtafThreadLocalClass.vtafThreadLocal.get().getCurrentTestCase().getTestSteps().add(testStep);
		// testCase.getTestSteps().add(testStep);

		if (!isPassed) {
			VtafThreadLocalClass.vtafThreadLocal.get().getCurrentTestCase().setResult("Failed");
			VtafThreadLocalClass.vtafThreadLocal.get().getCurrentTestSuit().setResult("Failed");
	 ...
	 */
	public final synchronized void setExecutionSummary() {

		for (TestSuite ts : testExecution.getTestSuites()) {
			for (TestCase tc : ts.getTestCases()) {
				int tcId = tc.getTestCaseId();
				// System.out.println("Anzar: " + tcId+ ": " + tc.getResult());
				if (!reportedTestCases.containsKey(tcId)) {
					reportedTestCases.put(tcId, tc.getResult());
				...
				} else { // Added for parallel running
					String result = reportedTestCases.get(tcId);
					if (!result.equals(tc.getResult())) {
						if ("Failed".equals(tc.getResult())) { // previously
																// Success
							testExecution.setTotalfailedcount(1);
							testExecution.setTotalerrorcount(1);
							testExecution.setTotalsuccesscount(-1);
						} else if ("Success".equals(tc.getResult())) { // Previously
																		// Failed
							testExecution.setTotalsuccesscount(1);
							testExecution.setTotalfailedcount(-1);
							testExecution.setTotalerrorcount(-1);
						}
						reportedTestCases.put(tcId, tc.getResult()); // Upgrade
																		// the
																		// result
					}
				}
		...
		String hmsEx = String.format("%02d:%02d:%02d", TimeUnit.MILLISECONDS.toHours(millisTotal),
				TimeUnit.MILLISECONDS.toMinutes(millisTotal)
						- TimeUnit.HOURS.toMinutes(TimeUnit.MILLISECONDS.toHours(millisTotal)),
				TimeUnit.MILLISECONDS.toSeconds(millisTotal)
						- TimeUnit.MINUTES.toSeconds(TimeUnit.MILLISECONDS.toMinutes(millisTotal)));
		System.out.println(hmsEx);

		testExecution.setTotaltime(hmsEx + " HH:MM:SS");
		System.out.println("Report created successfully to the folder " + getReportFolderLocation());

		long timeTsEnd = System.currentTimeMillis();
		long tsTime = timeTsEnd - timeStart;
		String hms = String.format("%02d:%02d:%02d", TimeUnit.MILLISECONDS.toHours(tsTime),
				TimeUnit.MILLISECONDS.toMinutes(tsTime)
						- TimeUnit.HOURS.toMinutes(TimeUnit.MILLISECONDS.toHours(tsTime)),
				TimeUnit.MILLISECONDS.toSeconds(tsTime)
						- TimeUnit.MINUTES.toSeconds(TimeUnit.MILLISECONDS.toMinutes(tsTime)));
