...
public class Reporter {
	...
	private String initReportDirectory() {
		...
		File reportFolder = new File("target" + File.separator + "Reports" + File.separator+ SeleneseTestNgHelperVir.getReleaseVersion() +  "_" + SeleneseTestNgHelperVir.getCurrentPlanName() + "_"+ SeleneseTestNgHelperVir.getBrowserString() + "_" + timestamp);


		try {
			if (!reportFolder.exists() && !reportFolder.mkdirs()) {
				throw new RuntimeException("Cannot create new folder in location " + reportFolder.getAbsolutePath());
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	private void copyReportHelperFiles(final String reportFolderStr) {
		File reportTemplateHtml = new File(new File(
				"src" + File.separator + "main" + File.separator + "resources" + File.separator + "ReportTemplate")
						.getAbsolutePath());
		/*
		 * File reportTemplateHtml = new File(new
		 * File("ReportTemplate").getAbsolutePath());
		 */
	 ...
	 *            the stacktrace
	 */
	public final void reportStepResults(WebDriver driver, final boolean isPassed, final String category,
			final String message, final String loglvl, final String stacktrace) {
		...
		if (isPassed) {
			...
			if ("SCREENSHOT ".equals(category)) {
				...
				try {
					...
					// saveFullScreenshotThumb(screenShot);
					// String thumbScreenShot = saveScreenshotThumb(screenShot);

				} catch (Exception e) {
					screenShot = saveScreenShot(builder.getReportFolderLocation());
				}

				String screenShot2 = saveScreenShot(builder.getReportFolderLocation());

				String thumbScreenShot = saveScreenshotThumb(screenShot2);
				builder.addNewTestStep(isPassed, category, "images" + File.separator + screenShot, thumbScreenShot,
						message, stacktrace, "Success");
			} else {
			...
			try {
				...
				if (EnableExtentReport.equalsIgnoreCase("yes")) {
					extentReportObj.getExtentTest().log(LogStatus.FAIL,category, "Failed due to missing element <br/>" +message);
	...
	private String saveFullScreenshotThumb(final String screenShotFile) {
		String screenShotThumbPath = "images" + File.separator + screenShotFile + "_Thumb.png";
		String screenShotThumb = builder.getReportFolderLocation() + File.separator + "images" + File.separator
				+ screenShotFile + "_Thumb.png";
		try {
	 ...
	 * End test reporting.
	 */
	public final synchronized void endTestReporting() {
	...
	public final ReportBuilder getBuilder() {
		return builder;
	}
